...
CREATE_GROUP(SAMPLE_SENSOROUTPUT_,
    static_cast<uint16_t>(SAMPLE_SENSOROUTPUT_GAPS),
    static_cast<uint16_t>(SAMPLE_SENSOROUTPUT_PROBESBAD),
    static_cast<uint16_t>(SAMPLE_SENSOROUTPUT_PROBESGOOD));
...
const DescriptorItem_t s_possibleSbemItems[] = {
    ...
    {SAMPLE_SENSOROUTPUT_GAPS, "<PTH>Sample+SensorOutput.gaps\n<FRM>uint32"},
    {SAMPLE_SENSOROUTPUT_PROBESBAD, "<PTH>Sample.SensorOutput.probesBad\n<FRM>uint32"},
    {SAMPLE_SENSOROUTPUT_PROBESGOOD, "<PTH>Sample.SensorOutput.probesGood\n<FRM>uint32"},
...
size_t writeToSbemBuffer_SAMPLE_SENSOROUTPUT(void *buffer, size_t bufferLen, size_t startOffset, const WB_RES::LOCAL::SAMPLE_SENSOROUTPUT::EVENT::NotificationType &data){
    size_t bytesRead = 0;
    size_t bytesWritten = 0;
    uint8_t *byteBuf = static_cast<uint8_t*>(buffer);
    {
        auto value = data.gaps;
        const uint8_t *src = reinterpret_cast<const uint8_t*>(&value);
        for(size_t i=0; i<sizeof(value); i++, src++, bytesRead++)
            if ((bytesRead >= startOffset) && (bytesWritten < bufferLen)) byteBuf[bytesWritten++] = *src;
    }
    {
        auto value = data.probesBad;
        const uint8_t *src = reinterpret_cast<const uint8_t*>(&value);
        for(size_t i=0; i<sizeof(value); i++, src++, bytesRead++)
            if ((bytesRead >= startOffset) && (bytesWritten < bufferLen)) byteBuf[bytesWritten++] = *src;
    }
    {
        auto value = data.probesGood;
        const uint8_t *src = reinterpret_cast<const uint8_t*>(&value);
        for(size_t i=0; i<sizeof(value); i++, src++, bytesRead++)
            if ((bytesRead >= startOffset) && (bytesWritten < bufferLen)) byteBuf[bytesWritten++] = *src;
    }
...
size_t getSbemLength_SAMPLE_SENSOROUTPUT(const WB_RES::LOCAL::SAMPLE_SENSOROUTPUT::EVENT::NotificationType &data)
{
    return (0 + 4 + 4 + 4);
}
...
size_t getSbemLength(whiteboard::LocalResourceId localResId, const whiteboard::Value &data)
{
    ...
    case WB_RES::LOCAL::SAMPLE_SENSOROUTPUT::LID:
        return getSbemLength_SAMPLE_SENSOROUTPUT(data.convertTo<WB_RES::LOCAL::SAMPLE_SENSOROUTPUT::EVENT::NotificationType>());
    case WB_RES::LOCAL::TIME::LID:
...
size_t writeToSbemBuffer(void *buffer, size_t bufferLen, size_t startOffset, whiteboard::LocalResourceId localResId, const whiteboard::Value &data)
{
    ...
    case WB_RES::LOCAL::SAMPLE_SENSOROUTPUT::LID:
        return writeToSbemBuffer_SAMPLE_SENSOROUTPUT(buffer, bufferLen, startOffset, data.convertTo<WB_RES::LOCAL::SAMPLE_SENSOROUTPUT::EVENT::NotificationType>());
    case WB_RES::LOCAL::TIME::LID:
...
const SbemResID2GrpIdMapping s_sbemResID2GrpIdMap[] = {
    ...
    {WB_RES::LOCAL::SAMPLE_SENSOROUTPUT::LID,  + 4 + 4 + 4, SAMPLE_SENSOROUTPUT__Group },
